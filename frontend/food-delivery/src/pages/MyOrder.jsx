...
if(data.success){
  ...
  return (
    <div className='mt-12 pb-16'>
      <div>
        <p className='text-2xl font-medium md:text-3xl'>My Order</p>
      </div>
      {myOrder.map((order,index)=>(
        ...
        '>
          ...
          {order.items.map((item, index)=>(
            <div key={index}  className={`relative bg-white text-gray-800 ${
          order.items.length !== index + 1 ? "border-b" : ""
        } border-gray-300 flex flex-col md:flex-row md:items-center 
        justify-between p-4 py-5 w-full max-w-4xl`}>
               <div className='flex items-center mb-4 md:mb-0'>
                <div className='p-4 rounded-lg'>
                  <img src={`http://localhost:3001/images/${item.product.image[0] }`} alt="" className='w-16' />
                </div>
                <div className='ml-4'>
